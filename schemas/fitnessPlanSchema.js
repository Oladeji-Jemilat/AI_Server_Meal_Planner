...
const summarySchema = z.object({
    goal: z.enum(["lose weight", "maintain weight", "gain weight"]),
    durationDays: z.coerce.number().positive().int().nullable().optional(),
    timePerDay: z.string(),
    dietPreference: z.enum([
        "none",
        "balanced",
...
const weekSchema = z.object({
    week: z.coerce.number().positive().int(),

    workout: z.array(workoutSchema).default([]),

    nutrition: z.object({
        dailyCalories: z.coerce.number().positive().int(),
        macros: z.object({
            protein: z.string(),
            carbs: z.string(),
            fat: z.string(),
...
const aiPlanSchema = z.object({
    summary: summarySchema,
    weeklyPlan: z.array(weekSchema).default([]),
